...
import type { StyleProfile, ResearchData, ScriptOutput, Scene, CalibrationData } from '../pipelineTypes.js';
import type {
...
import { createLogger } from '../libFacade.js';
...
export function parseStyleAnalysisCIR(
  ...
  const cir: StyleAnalysisCIR = {
    ...
    packagingTrack: {
      ...
      subtitleFontSize: normaliseSubtitleFontSize(coerceToString(trackD.subtitle_font_size)),
...
/** Coerce a field that may be a number (e.g. subtitle_font_size: 18) to a string for downstream normalisation. */
function coerceToString(v: string | number | undefined): string | undefined {
...
export function buildVideoIR(options: BuildVideoIROptions): VideoIR {
  ...
  const scenes: VideoIRScene[] = storyboardCIR.scenes.map((sb, i) => {
    ...
    if (!tp) {
      throw new Error(`buildVideoIR: missing temporalPlan entry for scene index ${i}`);
    }
    ...
    if (shotTransitions.length > 0) {
      ...
      const shot = shotTransitions[shotIdx];
      if (shot) transitionToNext = shot.transitionToNext;
