...
// Stub aiControl + the underlying abort primitives so retry waits stay fast.
vi.mock('@ai-video/pipeline-core/aiControl.js', () => ({
  runWithAICallControl: (fn: () => Promise<any>) => fn(),
  throwIfAborted: () => {},
  waitWithAbort: (ms: number) => new Promise(r => setTimeout(r, Math.min(ms, 10))),
}));
vi.mock('@ai-video/lib/abortable.js', () => ({
  throwIfAborted: () => {},
  waitWithAbort: (ms: number) => new Promise(r => setTimeout(r, Math.min(ms, 10))),
  AIRequestAbortedError: class extends Error {},
}));
