...
export class ConfigStore {
  ...
  constructor(dataDir: string) {
    ...
    console.log(`[ConfigStore] Loaded from ${this.filePath}: keys=${Object.keys(this.config).join(',')}, hasVideoProvider=${!!this.config.videoProviderConfig}, hasGemini=${!!this.config.geminiApiKey}`);
  }

  private load(): AppConfig {
    if (!existsSync(this.filePath)) {
      console.warn(`[ConfigStore] Config file not found: ${this.filePath}`);
      return {};
    }
    try {
      ...
      console.error(`[ConfigStore] Failed to parse ${this.filePath}:`, err);
      return {};
    }
  }
