...
Output JSON only:
{
  ...
  try {
    console.log('[CV_PREPROCESS] analyzing thumbnail:', thumbnailPath);
    const result = await adapter.generateText('', prompt, {
      responseMimeType: 'application/json',
    });
    console.log('[CV_PREPROCESS] raw response preview:', (result.text ?? '').slice(0, 500));

    const cvData = extractJSON<any>(result.text ?? '');
    console.log('[CV_PREPROCESS] parsed cvData:', cvData ? JSON.stringify(cvData).slice(0, 300) : 'null');
    if (cvData) {
