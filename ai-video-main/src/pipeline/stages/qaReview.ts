...
export async function runQaReview(
  ...
  const prompt = fillTemplate(QA_REVIEW_PROMPT, {
    ...
    reference_transcript_sample: (styleProfile.fullTranscript ?? '').slice(0, 500) || '(no reference transcript available)',
  });
  console.log('[QA_REVIEW] prompt preview:', prompt.slice(0, 500));

  const result = await adapter.generateText('', prompt, {
    responseMimeType: 'application/json',
  });
  console.log('[QA_REVIEW] raw response length:', (result.text ?? '').length);
  console.log('[QA_REVIEW] raw response preview:', (result.text ?? '').slice(0, 1000));

  const reviewData = extractJSON<any>(result.text ?? '');
  console.log('[QA_REVIEW] parsed keys:', reviewData ? Object.keys(reviewData).join(', ') : 'null');
