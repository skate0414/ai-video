...
export async function runResearch(
  ...
  console.log('[RESEARCH] prompt preview:', prompt.slice(0, 500));
  ...
  console.log('[RESEARCH] supportsGrounding:', supportsGrounding, 'provider:', adapter.provider);
  const result = await adapter.generateText('', prompt, {
    responseMimeType: 'application/json',
    ...(supportsGrounding ? { tools: [{ googleSearch: {} }] } : {}),
  });
  console.log('[RESEARCH] raw response length:', (result.text ?? '').length);
  console.log('[RESEARCH] raw response preview:', (result.text ?? '').slice(0, 1000));

  const researchData = extractJSON<any>(result.text ?? '');
  console.log('[RESEARCH] parsed keys:', researchData ? Object.keys(researchData).join(', ') : 'null');
  if (!researchData) {
