...
export async function runCapabilityAssessment(
  ...
  console.log('[CAPABILITY_ASSESSMENT] prompt:', safetyPrompt.slice(0, 500));
  const safetyResult = await adapter.generateText('', safetyPrompt, {
    responseMimeType: 'application/json',
  });
  console.log('[CAPABILITY_ASSESSMENT] raw response:', (safetyResult.text ?? '').slice(0, 1000));

  const safetyCheck = extractJSON<{ safe: boolean; reason?: string }>(safetyResult.text ?? '') ?? { safe: true };
  console.log('[CAPABILITY_ASSESSMENT] parsed safetyCheck:', JSON.stringify(safetyCheck));
  if (!safetyCheck.safe) {
