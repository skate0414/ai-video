...
Output JSON (no markdown):
{
  ...
  console.log('[NARRATIVE_MAP] prompt preview:', prompt.slice(0, 500));

  const result = await adapter.generateText('', prompt, {
    responseMimeType: 'application/json',
  });
  console.log('[NARRATIVE_MAP] raw response length:', (result.text ?? '').length);
  console.log('[NARRATIVE_MAP] raw response preview:', (result.text ?? '').slice(0, 1000));

  const data = extractJSON<any>(result.text ?? '');
  console.log('[NARRATIVE_MAP] parsed data keys:', data ? Object.keys(data).join(', ') : 'null');
  ...
  // Compute target scene count from reference video duration and scene_avg_duration
  const sceneAvgDuration = styleProfile.track_b_visual?.scene_avg_duration_sec ?? 5;
  const videoDuration = meta.video_duration_sec ?? 60;
  const targetSceneCount = Math.round(videoDuration / sceneAvgDuration);
  ...
  const generationPlan: GenerationPlan = {
    ...
    targetSceneDuration: sceneAvgDuration,
    ...
    reasoning: [
      ...
      `Target scene count: ${targetSceneCount} (${videoDuration}s / ${sceneAvgDuration}s per scene)`,
    ],
  };
