...
export async function runSubjectIsolation(
  ...
  console.log('[SUBJECT_ISOLATION] prompt preview:', prompt.slice(0, 500));
  console.log('[SUBJECT_ISOLATION] scenes count:', scenesForPrompt.length);

  const result = await adapter.generateText('', prompt, {
    responseMimeType: 'application/json',
  });
  console.log('[SUBJECT_ISOLATION] raw response length:', (result.text ?? '').length);
  console.log('[SUBJECT_ISOLATION] raw response preview:', (result.text ?? '').slice(0, 1000));

  const checkData = extractJSON<any>(result.text ?? '');
  console.log('[SUBJECT_ISOLATION] parsed results count:', checkData?.results?.length ?? 0);
