...
export const PROVIDER_PRESETS: Record<string, SiteAutomationConfig> = {
  ...
  seedance: {
    ...
    label: '即梦 (Jimeng)',
    type: 'video',
    siteUrl: 'https://jimeng.jianying.com/ai-tool/generate?workspace=0&type=video',
    capabilities: { video: true, fileUpload: true },
    selectors: {
      promptInput: [
        { selector: 'div.tiptap.ProseMirror[contenteditable="true"]', method: 'css', priority: 3 },
        { selector: '[contenteditable="true"]', method: 'css', priority: 2 },
      ],
      generateButton: [
        { selector: 'button[class*="submit-"]', method: 'css', priority: 3 },
        { selector: 'button.lv-btn-icon-only[class*="submit"]', method: 'css', priority: 2 },
      ],
      resultElement: [
        { selector: 'video', method: 'css', priority: 3 },
