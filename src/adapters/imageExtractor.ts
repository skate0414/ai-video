/* ------------------------------------------------------------------ */
/*  ImageExtractor – extracts codegen output (images) from chat DOM   */
...
export async function extractLatestImage(
  ...
  /** Optional SelectorChain for resilient response block resolution. */
  ...
  // If a SelectorChain is provided, resolve it first for a more accurate selector
  let resolvedSelector = responseBlockSelector;
  if (responseBlockChain && responseBlockChain.length > 0) {
    try {
      const resolved = await resolveSelector(page, responseBlockChain, 3_000);
      if (resolved) {
        // Use the matched CSS selector from the chain
        resolvedSelector = resolved.strategy.method === 'css'
          ? resolved.strategy.selector
          : responseBlockSelector;
        console.log(`[ImageExtractor] Resolved response block via SelectorChain: ${resolvedSelector}`);
      }
    } catch {
      // fall back to flat selector
    }
  }
