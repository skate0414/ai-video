...
export class FallbackAdapter implements AIAdapter {
  ...
  async generateText(
    ...
    try {
      ...
      if (isQuotaError(err)) {
        console.log(`[FallbackAdapter] generateText quota error on ${this.primary.provider}, falling back to ${this.fallback.provider}`);
        return this.fallback.generateText(model, prompt, options);
      }
      throw err;
    }
  }
  ...
  async generateImage(
    ...
    try {
      ...
      if (isQuotaError(err)) {
        console.log(`[FallbackAdapter] generateImage quota error on ${this.primary.provider}, falling back to ${this.fallback.provider}`);
  ...
  async generateVideo(
    ...
    try {
      ...
      if (isQuotaError(err)) {
        console.log(`[FallbackAdapter] generateVideo quota error on ${this.primary.provider}, falling back to ${this.fallback.provider}`);
