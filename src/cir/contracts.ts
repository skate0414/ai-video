...
/**
 * Validate a StyleAnalysisCIR has all required fields populated.
 */
...
/**
 * Validate a FormatSignature artifact (not a CIR — uses _type instead of _cir).
...
/**
 * Validate a VideoIR — the compiler's MIR.
 *
 * Zero optional fields allowed: every scene must be fully resolved
 * with timing, voice, rate, and visual content. Any undefined field
 * is a compile error.
 */
export function validateVideoIR(cir: unknown): string[] {
  ...
  for (let i = 0; i < scenes.length; i++) {
    ...
    // ttsRate can be undefined (means default rate), but must be string if present
