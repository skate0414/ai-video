...
import type { ProviderId, ChatMode, ProviderSelectors, AiResourceType } from '../types.js';
import {
  ...
  return [
    ...
    /* ---- Tasks ---- */
    {
      ...
      handler: async (req, res) => {
        ...
        const tasks = workbench.tasks.add(
          ...
          body.preferredModel,
          body.attachments,
          { chatMode: workbench.getState().chatMode },
        );
        json(res, 201, tasks);
      },
    },
    ...
    /* ---- Accounts ---- */
    {
      ...
      handler: async (req, res) => {
        ...
        const acc = workbench.resources.addAccount(body.provider, body.label, body.profileDir);
        json(res, 201, acc);
      },
    },
    {
      ...
      handler: (_req, res, match) => {
        const ok = workbench.resources.removeAccount(match.groups!.id);
        json(res, ok ? 200 : 404, { ok });
      },
    },
    {
      ...
      handler: (_req, res) => {
        workbench.resources.resetAllQuotas();
        json(res, 200, { ok: true });
      },
    },
    {
    ...
    /* ---- Unified Resources ---- */
    {
      ...
      handler: (_req, res) => {
        json(res, 200, workbench.resources.all());
      },
    },
    {
      method: 'GET',
      pattern: /^\/api\/resources\/by-type\/(?<type>[^/]+)$/,
      handler: (_req, res, match) => {
        const type = match.groups!.type as AiResourceType;
        json(res, 200, workbench.resources.byType(type));
      },
    },
    {
      method: 'POST',
      ...
      handler: async (req, res) => {
        ...
        if (!body.type || !body.provider || !body.label || !body.siteUrl) {
          return json(res, 400, { error: 'type, provider, label, and siteUrl are required' });
        }
        const resource = workbench.resources.addResource(body);
    ...
    },
    {
      ...
      handler: (_req, res, match) => {
        const ok = workbench.resources.removeResource(match.groups!.id);
        json(res, ok ? 200 : 404, { ok });
      },
    },
    {
      method: 'POST',
      pattern: /^\/api\/resources\/(?<id>[^/]+)\/login$/,
      handler: async (_req, res, match) => {
        try {
          await workbench.openLoginBrowser(match.groups!.id);
          json(res, 200, { ok: true });
    ...
    {
      method: 'POST',
      pattern: /^\/api\/resources\/(?<id>[^/]+)\/close-login$/,
      handler: async (_req, res, match) => {
        await workbench.closeLoginBrowser(match.groups!.id);
        json(res, 200, { ok: true });
      },
    ...
    {
      method: 'POST',
      pattern: /^\/api\/resources\/reset-quotas$/,
      handler: (_req, res) => {
        workbench.resources.resetAllQuotas();
        json(res, 200, { ok: true });
      },
    ...
    },
    {
      ...
      handler: async (req, res) => {
        const body = await parseJsonBody<{ chatUrl: string; type?: string }>(req);
        if (!body.chatUrl?.trim()) return json(res, 400, { error: 'chatUrl is required' });
        try {
          const result = await workbench.addProviderFromUrl(body.chatUrl.trim(), body.type?.trim() || undefined);
          json(res, 201, result);
        } catch (err) {
          json(res, 400, { error: sanitizeError(err) });
        }
