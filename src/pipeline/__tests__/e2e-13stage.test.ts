...
vi.mock('../../adapters/ttsProvider.js', async (importOriginal) => {
...
vi.mock('../../adapters/ffmpegAssembler.js', async (importOriginal) => {
  const orig = await importOriginal() as Record<string, unknown>;
  return {
    ...orig,
...
import type { AIAdapter, PipelineEvent, GenerationResult, PipelineProject } from '../types.js';
