...
/*  Estimates cost and assigns backends to each compilation pass      */
...
import { resolveProvider, type QualityDecision } from './qualityRouter.js';
...
const STAGE_REQUIREMENTS: Record<PipelineStage, StageRequirement> = {
  ...
  QA_REVIEW: { text: true },
  TEMPORAL_PLANNING: {}, // pure computation — no AI provider needed
  STORYBOARD: { text: true },
  VIDEO_IR_COMPILE: {}, // pure computation — no AI provider needed
...
export function generateResourcePlan(
  ...
  for (const stage of ALL_STAGES) {
    ...
    // Get quality router decision (registry-aware)
