...
import { isEdgeTTSAvailable, type TTSConfig } from '../../../adapters/ttsProvider.js';
...
registerStage({
  ...
  async execute(ctx: StageRunContext) {
    ...
    const { resolveFormatPreset, getFormatName } = await import('../formatPresets.js');
    ...
    const { resolveEncodingProfile, getQualityTier } = await import('../encodingProfiles.js');
    ...
    const { computeAdaptiveTransitions } = await import('../adaptiveTransitions.js');
    ...
    const { buildColorGradeFilter } = await import('../colorGrading.js');
    ...
    const refineOptions = (project as any).refineOptions as import('../../../../shared/types.js').RefineOptions | undefined;
    ...
    const { getVideoInfo } = await import('../../../adapters/ffmpegAssembler.js');
    ...
    try {
      const { computePostAssemblyMetrics } = await import('../postAssemblyQuality.js');
    ...
    try {
      const { extractFrame, computeTemporalMetrics } = await import('../temporalQuality.js');
      ...
      const scenePaths = sceneDurations.map((_d, i) => {
        const s = project.scenes![i];
        return s.assetUrl ? join(assetsDir, s.assetUrl.replace(/^.*[\\/]/, '')) : '';
      });

      // Build pseudo scene video paths for temporal analysis  
      // We extract frames from the final video at scene boundary times
      ...
      for (let i = 0; i < sceneDurations.length - 1; i++) {
        ...
        // Use cvMetrics SSIM for consistency
        const { computeSSIM } = await import('../cvMetrics.js');
