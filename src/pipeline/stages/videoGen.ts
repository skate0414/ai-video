...
export interface VideoGenInput {
  /** Mutable scene array — VIDEO_GEN writes assetUrl/status back. */
  scenes: Scene[];
  /** Compiled VideoIR — authoritative source for durations and asset decisions. */
  videoIR: VideoIR;
  assetsDir: string;
  /** aivideomaker.ai API adapters for parallel video generation. */
  aivideomakerAdapters?: AIAdapter[];
  /** Number of candidate videos to generate per scene; best is picked by quality score (default 1). */
  candidateCount?: number;
}

const log = createStageLog('VIDEO_GEN');

const VIDEO_MAX_RETRIES = getRetryBudget(); // configurable via SCENE_MAX_RETRIES env (default 2)
const VIDEO_RETRY_DELAY_MS = 10_000; // base delay for legacy reference; actual delay uses exponential backoff

/**
 * Generate videos from keyframe images for scenes with assetType === 'video'.
 * Uses aivideomaker.ai API (i2v model) for video generation.
...
export async function runVideoGen(
  ...
  // Sync asset types from VideoIR — compile pass is the authority for image/video decisions
  ...
  for (let i = 0; i < results.length; i++) {
    if (results[i].assetType === 'video') {
      // Skip scenes that already have a video file on disk (from prior partial runs)
  ...
  // Style anchor: extract VisualDNA from reference sheet for prompt consistency
  let styleAnchor = '';
  ...
  // ── Quota-aware worker pool ──
  // Shared queue that supports returning scenes when a worker hits quota limits.
  // Workers marked as depleted stop claiming new work; their uncompleted scenes
  // go back to the queue for healthy workers to pick up.
  ...
  async function worker(workerAdapter: AIAdapter, workerId: number) {
    ...
    while (true) {
      ...
      for (let attempt = 0; attempt <= VIDEO_MAX_RETRIES; attempt++) {
        ...
        try {
          ...
          if (candidateCount > 1 && attempt === 0) {
            // Multi-candidate: generate N candidates, keep the one with the best visual score.
          ...
          if (results[index].assetType === 'video' && results[index].assetUrl) {
            // Post-generation visual quality check (use keyframe or reference image)
            let sceneDna;
            try {
              const dnaSource = results[index].keyframeUrl ?? results[index].referenceImageUrl;
              if (dnaSource) {
                sceneDna = await extractVisualDNA(adapter, dnaSource, emit);
              }
            } catch (err) {
              slog.warn('dna_extract_failed', { scene: scene.number, err: err instanceof Error ? err.message : String(err) });
            }

            // compute visual score and decide whether to retry
            if (refDNA && sceneDna) {
              ...
              // Compute CV metrics (SSIM + sharpness) when source image available
              let cvMetrics: CVQualityMetrics | undefined;
              const dnaSource = results[index].keyframeUrl ?? results[index].referenceImageUrl;
              if (dnaSource && existsSync(refSheetPath)) {
                try { cvMetrics = await computeCVMetrics(dnaSource, refSheetPath); } catch { /* non-fatal */ }
              }
              ...
              // degrade if overall remains low after attempts
          ...
          // Detect quota exhaustion — stop this worker and return scene to queue
          if ((err as any)?.isQuotaError) {
            ...
            quotaDepleted = true;
            // Reset scene state so another worker can attempt it cleanly
            results[index].status = 'pending';
            results[index].progressMessage = undefined;
            results[index].assetUrl = undefined as any;
      ...
      if (quotaDepleted) break; // stop this worker entirely
      if (!succeeded) {
        // Degrade to static image with Ken Burns effect — use keyframe or reference image
  ...
  if (adapters.length > 0 && videoScenes.length > 0) {
    ...
    // After the first pass, some workers may have exited before quota-returned
    // scenes were added back. Re-run healthy workers on any remaining items.
    while (queueHead < queue.length && depletedWorkers.size < adapters.length) {
      const remaining = queue.length - queueHead;
      const healthyAdapters = adapters
        .map((a, i) => ({ adapter: a, id: i }))
        .filter(w => !depletedWorkers.has(w.id));
    ...
    // If all workers are quota-depleted but scenes remain in the queue,
    // those scenes were returned but nobody could pick them up — degrade them.
  ...
  // Fail fast: if we attempted video gen but produced zero videos, throw so
  // the stage surfaces as an error instead of silently degrading to images.
...
async function generateSceneVideo(
  ...
  // ── Tier 1: aivideomaker.ai API (i2v, cheapest/free) ──
  if (aivideomakerAdapter) {
    try {
      ...
      if (videoResult.videoUrl) {
        // Save base64 data URL or remote URL to a local file
      ...
      // Re-throw quota errors so the worker pool can route to another account
