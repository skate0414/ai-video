...
export interface FinalRiskGateInput {
  ...
  scriptText: string;
  /** If true, the script already passed LLM safety classification as a false positive. */
...
export function runFinalRiskGate(
  ...
  //    Skip keyword check if LLM already classified content as a false positive
  //    during SCRIPT_GENERATION (safetyPreCleared).
