...
const ROUTE_TABLE: RouteRule[] = [
  // 1. Capability Assessment (safety check)
  {
    ...
    premium:  { adapter: 'chat', reason: 'Safety check is simple classification' },
  },
  ...
  },
  {
    ...
    premium:  { adapter: 'api',  reason: 'API for structured verification' },
  },
  // 4. Narrative Map (includes calibration)
  {
    ...
    premium:  { adapter: 'chat', reason: 'Simple calculation task' },
  },
  // 5. Narrative Map (narrative structure)
  {
    ...
    premium:  { adapter: 'api',  reason: 'API for reliable narrative structure' },
  },
  // 6. Script Generation
  {
  ...
  // 7. QA Review
  {
    ...
    premium:  { adapter: 'api',  reason: 'API for structured review output' },
  },
  // 8. Storyboard
  {
    stage: 'STORYBOARD',
    ...
    premium:  { adapter: 'api',  reason: 'API for structured JSON storyboard' },
  },
  // 9. Reference Image
  {
    stage: 'REFERENCE_IMAGE',
    taskType: 'image_generation',
    free:     { adapter: 'chat', provider: 'gemini', reason: 'Gemini free chat can generate images' },
    balanced: { adapter: 'chat', provider: 'gemini', reason: 'Free first, FallbackAdapter auto-fallback to API' },
  ...
  // 11. Video Gen — smart priority: most expensive resource
  {
    ...
    free:     { adapter: 'chat', provider: 'seedance', reason: 'Use Seedance web for video gen' },
...
export function routeTask(
  ...
  if (!rule) {
    ...
    if (qualityTier === 'premium') {
      return { adapter: 'api', reason: 'Default premium route' };
    }
