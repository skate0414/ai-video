...
export { SSE_EVENT } from '../../shared/types.js';
...
import type { PipelineStage, ProcessStatus, ModelOverrides, LogEntry } from '../../shared/types.js';
...
export interface Scene {
  ...
  logs: string[];
  /** Optional quality report computed during generation */
...
/* ---- Script Skeleton (intermediate between structure + writing) ---- */

/** Skeleton sentence – structure only, no creative text yet. */
...
/** Output of Step A (skeleton generation). */
...
export interface PipelineProject {
  ...
  stageProviderOverrides?: import('../../shared/types.js').StageProviderOverrides;

  /** Per-prompt text overrides — key is prompt constant name (e.g. "WRITING_SYSTEM_PROMPT") */
  promptOverrides?: Record<string, string>;

  /** Transient directive injected via retryStage — consumed and cleared on next execution */
  retryDirective?: { stage: PipelineStage; directive: string; timestamp: string };

  /** Optional storyboard replication scaffold settings. */
  ...
  /** Temporal plan (sentence-level duration allocation) */
  temporalPlan?: import('../cir/types.js').TemporalPlanCIR;

  /** Video IR — fully-resolved production plan (compiler MIR). Immutable after VIDEO_IR_COMPILE. */
  videoIR?: import('../cir/types.js').VideoIR;

  /** QA review result */
  qaReviewResult?: { approved: boolean; feedback?: string };
  /** Flagged when QA exhausts all retries without approval */
